...
class CircleChanger(object):
    ...
    def swell_or_shrink_repeatedly(self,
        ...
        for k in range(times_to_swell_or_shrink):
            radiusstart = self.circle.radius
            self.swell_or_shrink_once(amount_to_swell_or_shrink)
            self.draw()
            self.circle.radius = radiusstart
            self.draw()
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        newcenter = self.circle.center.halfway_to(
            other_circle_changer.circle.center)
        newradius = (self.circle.center.get_distance_from(
            other_circle_changer.circle.center))/2
        newcolors = self.colors + other_circle_changer.colors
        return CircleChanger(newcenter.x, newcenter.y, newradius, 'red',
                             newcolors)
